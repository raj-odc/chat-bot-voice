...
export async function POST(req: Request) {
  try {
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a helpful assistant analyzing images. Describe what you see in detail.",
          },
          {
            role: "user",
            content: [
              {
                type: "text",
                text: "Describe this image in detail. What do you see? Focus on the main subjects and any relevant details.",
              },
              {
                type: "image_url",
                image_url: {
      ...
      return NextResponse.json({ description: response.choices[0]?.message?.content || "No description available" })
    ...
    console.error("Unhandled error in analyze image API:", error)
